...
const {verify}=createRequire(import.meta.url)('../scripts/verify-locations.cjs');
it('online helper makes no business mutations, validates isolation and never logs secrets',async()=>{
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.includes('/locations?limit='))body={items:[]};
  else if(url.includes('/locations'))status=404;
  return new Response(JSON.stringify(body),{status});
 };
 await verify(config,request,(s:string)=>output.push(s));expect(output.at(-1)).toBe('PHASE_6_READ_ONLY_VERIFICATION: PASS');
