...
const {verify}=createRequire(import.meta.url)('../scripts/verify-recipes.cjs');
it('online helper makes no business mutations, validates isolation and never logs secrets',async()=>{
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.endsWith('/access'))body={permissions:[{code:'masterdata.read'},{code:'masterdata.manage'}]};
  else if(url.includes(foreign))status=403;
  else if(url.includes('/items/'))body={items:[]};
  else if(url.includes('/recipes?'))body=[];
  else if(url.includes('/recipes/'))status=404;
  return new Response(JSON.stringify(body),{status});
 };
 await verify(config,request,(s:string)=>output.push(s));expect(output.at(-1)).toBe('PHASE_5_READ_ONLY_VERIFICATION: PASS');
 expect(output.join('\n')).not.toMatch(/SECRET_KEY_SENTINEL|PRIVATE_EMAIL_SENTINEL|PASSWORD_SENTINEL|TOKEN_SENTINEL/);
 expect(config.password).toBe('');
 expect(calls.filter(c=>c.url.includes('/api/')).every(c=>c.options.method==='GET'||c.url.endsWith('/recipes/calculate')&&c.options.method==='POST')).toBe(true);
