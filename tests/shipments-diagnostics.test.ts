...
const {verify}=createRequire(import.meta.url)('../scripts/verify-shipments.cjs');
it('online helper makes no business mutations, validates isolation and never logs secrets',async()=>{
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.endsWith('/access'))body={permissions:['shipments.read','shipments.manage','shipments.dispatch','inventory.read','inventory.reserve'].map(code=>({code}))};
  else if(url.includes(foreign))status=403;
  else if(url.includes('/shipments')&&url.includes('?limit='))body={items:[]};
  else if(url.includes('/shipments'))status=404;
  return new Response(JSON.stringify(body),{status});
 };
 await verify(config,request,(s:string)=>output.push(s));expect(output.at(-1)).toBe('PHASE_18_READ_ONLY_VERIFICATION: PASS');
