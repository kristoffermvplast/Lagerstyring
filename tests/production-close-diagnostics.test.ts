...
const {verify}=createRequire(import.meta.url)('../scripts/verify-production-close.cjs');
it('online helper makes no business mutations, validates isolation and never logs secrets',async()=>{
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.endsWith('/access'))body={permissions:['production.read','inventory.read','production.return','production.close'].map(code=>({code}))};
 ...
 await verify(config,request,(s:string)=>output.push(s));expect(output.at(-1)).toBe('PHASE_13_READ_ONLY_VERIFICATION: PASS');
