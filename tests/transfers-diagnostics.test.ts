...
const {verify}=createRequire(import.meta.url)('../scripts/verify-transfers.cjs');
it('online helper makes no business mutations, validates isolation and never logs secrets',async()=>{
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.endsWith('/access'))body={permissions:[{code:'inventory.read'},{code:'inventory.transfer'}]};
  else if(url.includes(foreign))status=403;
  else if(url.includes('/transfers')&&url.includes('?limit='))body={items:[]};
  else if(url.includes('/transfers'))status=404;
  return new Response(JSON.stringify(body),{status});
 };
 await verify(config,request,(s:string)=>output.push(s));expect(output.at(-1)).toBe('PHASE_9_READ_ONLY_VERIFICATION: PASS');
