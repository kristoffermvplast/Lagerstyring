...
const {verify}=createRequire(import.meta.url)('../scripts/verify-reports.cjs');
...
async function run(wrongScope=false){
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.endsWith('/access'))body={permissions:['reports.read','reports.export','production.read','inventory.read','shipments.read'].map(code=>({code}))};
  else if(url.includes(foreign))status=403;
  else if(url.includes('/unknown-report'))status=404;
  else if(url.includes('/reports/'))body={company_id:wrongScope?foreign:company,report:new URL(url).pathname.split('/').at(-1),items:[],totals:[],total:0};
...
it('verifies hosted reads without business writes or secret output',async()=>{expect(await run()).toContain('PHASE_22_READ_ONLY_VERIFICATION: PASS');});
it('fails closed on a foreign response and cleans up the session',async()=>{const messages=await run(true);expect(messages).toContain('REPORT_RESPONSE_SCOPE: FAIL');expect(messages).not.toContain('PHASE_22_READ_ONLY_VERIFICATION: PASS');});
