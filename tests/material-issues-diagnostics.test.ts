...
const {verify}=createRequire(import.meta.url)('../scripts/verify-material-issues.cjs');
it('online helper makes no business mutations, validates isolation and never logs secrets',async()=>{
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.endsWith('/access'))body={permissions:['production.read','production.issue','inventory.read','inventory.transfer'].map(code=>({code}))};
 ...
 await verify(config,request,(s:string)=>output.push(s));expect(output.at(-1)).toBe('PHASE_11_READ_ONLY_VERIFICATION: PASS');
