...
const {verify}=createRequire(import.meta.url)('../scripts/verify-production-waste.cjs');
it('online helper makes no business mutations, validates isolation and never logs secrets',async()=>{
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.endsWith('/access'))body={permissions:['production.read','inventory.read','production.waste','production.waste.correct'].map(code=>({code}))};
  else if(url.includes(foreign))status=403;
  else if(url.includes('/production-analysis'))body={items:[]};
 ...
 await verify(config,request,(s:string)=>output.push(s));expect(output.at(-1)).toBe('PHASE_14_READ_ONLY_VERIFICATION: PASS');
