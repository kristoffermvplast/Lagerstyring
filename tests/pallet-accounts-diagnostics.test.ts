...
const {verify}=createRequire(import.meta.url)('../scripts/verify-pallet-accounts.cjs');
it('online helper makes no business mutations, validates isolation and never logs secrets',async()=>{
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.endsWith('/access'))body={permissions:['pallets.read','pallets.manage','pallets.adjust'].map(code=>({code}))};
  else if(url.includes(foreign))status=403;
  else if(url.includes('/pallet-accounts')&&url.includes('?limit='))body={items:[]};
  else if(url.includes('/pallet-accounts'))status=404;
  return new Response(JSON.stringify(body),{status});
 };
 await verify(config,request,(s:string)=>output.push(s));expect(output.at(-1)).toBe('PHASE_19_READ_ONLY_VERIFICATION: PASS');
