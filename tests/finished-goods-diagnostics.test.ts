...
const {verify}=createRequire(import.meta.url)('../scripts/verify-finished-goods.cjs');
it('online helper makes no business mutations, validates isolation and never logs secrets',async()=>{
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.endsWith('/access'))body={permissions:['production.read','inventory.read','production.deliver','production.delivery.correct'].map(code=>({code}))};
  else if(url.includes(foreign))status=403;
  else if(url.includes('/handling-units'))body={items:[]};
 ...
 await verify(config,request,(s:string)=>output.push(s));expect(output.at(-1)).toBe('PHASE_15_READ_ONLY_VERIFICATION: PASS');
