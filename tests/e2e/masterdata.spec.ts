...
test('read-only users have no mutation controls',async({page})=>{
 await fixture(page,false);await openWorkspace(page,'Kunder');await expect(page.getByRole('button',{name:'Opret ny'})).toHaveCount(0);await expect(page.getByText(/Ingen resultater/)).toBeVisible();
});
test('company switch unmounts drafts and does not show the previous company records',async({page})=>{
 await fixture(page);await openWorkspace(page,'Kunder');await page.getByRole('button',{name:'Opret ny'}).click();await page.getByLabel('Nummer / kode',{exact:true}).fill('ONLY-A');await page.getByLabel('Navn',{exact:true}).fill('Only company A');await page.getByRole('button',{name:'Gem',exact:true}).click();await expect(page.getByRole('cell',{name:'Only company A'})).toBeVisible();await page.getByLabel('Virksomhed',{exact:true}).selectOption(other);await openWorkspace(page,'Kunder');await expect(page.getByRole('cell',{name:'Only company A'})).toHaveCount(0);await expect(page.getByText(/Ingen resultater/)).toBeVisible();
});
