...
async function fixture(page:Page,transfer=true){
 ...
 await page.route('**/api/companies/*/access',r=>r.fulfill({json:{permissions:[{code:'inventory.read'},{code:'production.read'},{code:'masterdata.read'},...(transfer?[{code:'inventory.transfer'},{code:'production.issue'}]:[])]}}));
 ...
 await page.route(/\/api\/companies\/[^/]+\/production-orders/,async r=>{
  ...
  const order={id:row.id,company_id:company,code:'PO-TEST',product:row,status:'planned',quantity:'10',version:2,problem:'',snapshot:{product:row,machine:row,unit:{symbol:'stk.'}},warnings:[],requirements:null};
...
test('issues to order with machine destination and retries the identical request',async({page})=>{
 const {row,bodies}=await fixture(page);await page.getByRole('button',{name:'Send materiale til produktion',exact:true}).click();
 await page.getByLabel('Fra beholdning',{exact:true}).selectOption({index:1});await expect(page.getByLabel('Tilplacering',{exact:true})).toHaveValue(row.id);await page.getByLabel('Mængde',{exact:true}).fill('1,00825001');
 page.on('dialog',d=>void d.accept());await page.getByRole('button',{name:'Bekræft lagerflytning'}).click();await expect(page.getByLabel('Mængde',{exact:true})).toBeDisabled();await page.getByRole('button',{name:'Prøv samme flytning igen'}).click();
 await expect(page.getByLabel('Mængde',{exact:true})).toHaveCount(0);expect(bodies).toHaveLength(2);expect(bodies[0]).toEqual(bodies[1]);expect(bodies[0].quantity).toBe('1.00825001');expect(bodies[0].owner_id).toBe(row.id);expect(bodies[0].reference).toBeUndefined();
});
