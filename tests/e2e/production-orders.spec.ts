...
test('stage4 minimal draft warns about missing setup without inventing defaults',async({page})=>{
 const {bodies}=await fixture(page);await page.route('**/defaults/*',r=>r.fulfill({json:{customer_id:null,machine_id:null,bom_revision_id:null,packing_revision_id:null,notes:'',recipes:[]}}));
 await page.getByRole('button',{name:'Ny produktionsordre',exact:true}).click();await chooseReference(page,'Vare',id);
 await expect(page.getByText(/Opsætning mangler:/)).toBeVisible();await expect(page.getByLabel('Maskine',{exact:true})).not.toBeVisible();
 await page.getByLabel('Ordrenummer',{exact:true}).fill('MISSING');await page.getByLabel('Planlagt antal',{exact:true}).fill('2');await page.getByRole('button',{name:'Gem kladde',exact:true}).click();await expect(page.getByRole('alert')).toBeVisible();expect(bodies[0].machine_id).toBeNull();expect(bodies[0].bom_revision_id).toBeNull();expect(bodies[0].packing_revision_id).toBeNull();
});
