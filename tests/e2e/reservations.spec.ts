...
async function fixture(page:Page,transfer=true){
 ...
 await page.route('**/api/companies/*/access',r=>r.fulfill({json:{permissions:[{code:'inventory.read'},{code:'masterdata.read'},...(transfer?[{code:'inventory.reserve'}]:[])]}}));
 ...
 await page.route(/\/api\/companies\/[^/]+\/reservations/,async r=>{
...
test('reserves through backend with confirmation and identical retry after uncertain response',async({page})=>{
 ...
 expect(bodies).toHaveLength(2);expect(bodies[0]).toEqual(bodies[1]);expect(bodies[0].quantity).toBe('1.00825001');expect(bodies[0].handling_unit_id).toBeNull();expect(confirmations).toBe(1);
});
