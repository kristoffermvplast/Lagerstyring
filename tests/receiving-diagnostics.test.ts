...
const {verify}=createRequire(import.meta.url)('../scripts/verify-receiving.cjs');
it('online helper makes no business mutations, validates isolation and never logs secrets',async()=>{
 ...
 const request=async(url:string,options:any)=>{
  ...
  else if(url.endsWith('/access'))body={permissions:[{code:'inventory.read'},{code:'inventory.receive'}]};
  else if(url.includes(foreign))status=403;
  else if(url.includes('/receipts')&&url.includes('?limit='))body={items:[]};
  else if(url.includes('/receipts'))status=404;
  return new Response(JSON.stringify(body),{status});
 };
 await verify(config,request,(s:string)=>output.push(s));expect(output.at(-1)).toBe('PHASE_8_READ_ONLY_VERIFICATION: PASS');
