...
async function verify(config, request = fetch, report = console.log) {
  ...
  try {
    ...
    requirePass(['shipments.read','shipments.manage','shipments.dispatch','inventory.read','inventory.reserve'].every(code => permissions.permissions?.some(p => p.code === code)), 'SHIPMENT_PERMISSIONS', report);
    ...
    const prefix=`/companies/${membership.company_id}/shipments`,foreignPrefix=`/companies/${foreign}/shipments`;
    const response=await api(prefix+'?limit=1',token);requirePass(response.status===200,'SHIPMENTS_READ',report);
    const body=await response.json();requirePass(Array.isArray(body.items)&&body.items.every(r=>r.company_id===membership.company_id),'SHIPMENTS_SCOPE',report);
    const probe='00000000-0000-4000-8000-000000000000';
    for(const suffix of ['','/'+probe])requirePass((await api(foreignPrefix+suffix,token)).status===403,'SHIPMENTS_FOREIGN_DENIED',report);
    requirePass((await api(prefix+'/'+probe,token)).status===404,'UNKNOWN_SHIPMENT_NOT_FOUND',report);
    if(body.items[0]){const response=await api(prefix+'/'+body.items[0].id,token);requirePass(response.status===200,'SHIPMENT_DETAIL_READ',report);const detail=await response.json();requirePass(detail.company_id===membership.company_id&&detail.id===body.items[0].id&&Array.isArray(detail.events)&&Array.isArray(detail.reservations)&&detail.reservations.every(e=>e.company_id===membership.company_id&&e.shipment_id===detail.id),'SHIPMENT_DETAIL_SCOPE',report);}
    else report('SHIPMENT_EXISTING_RECORD: NOT_RUN (no existing shipment; no fixture created)');
  ...
  report('PHASE_18_READ_ONLY_VERIFICATION: PASS');
}
async function main() {
  ...
  try {
    console.log('SHIPMENTS_VERIFICATION_VERSION: 1');
