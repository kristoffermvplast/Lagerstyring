...
async function verify(config, request = fetch, report = console.log) {
  ...
  try {
    ...
    requirePass(['production.read','production.manage'].every(code => permissions.permissions?.some(p => p.code === code)), 'PRODUCTION_ORDER_PERMISSIONS', report);
    ...
    if(body.items[0]) {
      ...
      requirePass(detail.status===200,'PRODUCTION_ORDER_DETAIL_READ',report);
      const data=await detail.json();
      requirePass(data.company_id===membership.company_id&&data.id===body.items[0].id&&typeof data.quantity==='string'&&data.snapshot?.product&&Array.isArray(data.warnings),'PRODUCTION_ORDER_DETAIL_SCOPE',report);
    } else report('PRODUCTION_ORDER_EXISTING_RECORD: NOT_RUN (no existing production order; no fixture created)');
    const probe='00000000-0000-4000-8000-000000000000';
    requirePass((await api(prefix+'/'+probe,token)).status===404,'UNKNOWN_PRODUCTION_ORDER_NOT_FOUND',report);
    requirePass((await api(foreignPrefix+'/'+probe,token)).status===403,'PRODUCTION_ORDER_DETAIL_FOREIGN_DENIED',report);
  ...
  report('PHASE_10_READ_ONLY_VERIFICATION: PASS');
}
async function main() {
  ...
  try {
    console.log('PRODUCTION_ORDER_VERIFICATION_VERSION: 1');
