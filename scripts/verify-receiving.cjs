...
async function verify(config, request = fetch, report = console.log) {
  ...
  try {
    ...
    requirePass(['inventory.read','inventory.receive'].every(code => permissions.permissions?.some(p => p.code === code)), 'RECEIVING_PERMISSIONS', report);
    ...
    const prefix = `/companies/${membership.company_id}/receipts`;
    const foreignPrefix = `/companies/${foreign}/receipts`;
    const response=await api(prefix+'?limit=1',token);
    requirePass(response.status===200,'RECEIPTS_READ',report);
    const body=await response.json();
    requirePass(Array.isArray(body.items)&&body.items.every(row=>row.company_id===membership.company_id&&typeof row.quantity==='string'&&row.snapshot),'RECEIPTS_SCOPE',report);
    requirePass((await api(foreignPrefix,token)).status===403,'RECEIPTS_FOREIGN_DENIED',report);
    if(body.items[0]) {
      const detail=await api(prefix+'/'+body.items[0].id,token);
      requirePass(detail.status===200,'RECEIPT_DETAIL_READ',report);
      const data=await detail.json();
      requirePass(data.company_id===membership.company_id&&data.id===body.items[0].id&&typeof data.quantity==='string'&&data.snapshot,'RECEIPT_DETAIL_SCOPE',report);
    } else report('RECEIPT_EXISTING_RECORD: NOT_RUN (no existing receipt; no fixture created)');
    const probe='00000000-0000-4000-8000-000000000000';
    requirePass((await api(prefix+'/'+probe,token)).status===404,'UNKNOWN_RECEIPT_NOT_FOUND',report);
    requirePass((await api(foreignPrefix+'/'+probe,token)).status===403,'RECEIPT_DETAIL_FOREIGN_DENIED',report);
  ...
  report('PHASE_8_READ_ONLY_VERIFICATION: PASS');
}
async function main() {
  ...
  try {
    console.log('RECEIVING_VERIFICATION_VERSION: 1');
