...
async function verify(config, request = fetch, report = console.log) {
  ...
  try {
    ...
    requirePass(['production.read','production.record','production.correct'].every(code => permissions.permissions?.some(p => p.code === code)), 'PRODUCTION_REGISTRATION_PERMISSIONS', report);
    ...
    const issuePrefix=prefix+'/'+orderId+'/registrations';
    for(const suffix of ['', '/summary', '/'+probe])requirePass((await api(foreignPrefix+'/'+orderId+'/registrations'+suffix,token)).status===403,'PRODUCTION_REGISTRATION_FOREIGN_DENIED'+(suffix==='/summary'?'_SUMMARY':suffix?'_DETAIL':'_LIST'),report);
    if(body.items[0]) {
      const listResponse=await api(issuePrefix,token);requirePass(listResponse.status===200,'PRODUCTION_REGISTRATIONS_READ',report);
      const list=await listResponse.json();requirePass(Array.isArray(list.items)&&list.items.every(e=>e.company_id===membership.company_id&&e.order_id===orderId),'PRODUCTION_REGISTRATIONS_SCOPE',report);
      const summaryResponse=await api(issuePrefix+'/summary',token);requirePass(summaryResponse.status===200,'PRODUCTION_REGISTRATION_SUMMARY',report);const summary=await summaryResponse.json();requirePass(summary.company_id===membership.company_id&&summary.order_id===orderId&&typeof summary.good_quantity==='string','PRODUCTION_REGISTRATION_SUMMARY_SCOPE',report);
      if(list.items[0]){const response=await api(issuePrefix+'/'+list.items[0].id,token);requirePass(response.status===200,'PRODUCTION_REGISTRATION_DETAIL',report);const entry=await response.json();requirePass(entry.company_id===membership.company_id&&entry.order_id===orderId&&entry.snapshot?.order_id===orderId,'PRODUCTION_REGISTRATION_DETAIL_SCOPE',report);}
      else report('PRODUCTION_REGISTRATION_EXISTING_RECORD: NOT_RUN (no existing production registration; no fixture created)');
      requirePass((await api(issuePrefix+'/'+probe,token)).status===404,'UNKNOWN_PRODUCTION_REGISTRATION_NOT_FOUND',report);
    } else report('PRODUCTION_REGISTRATION_ORDER_FIXTURE: NOT_RUN (no existing production order; no fixture created)');
    requirePass((await api(prefix+'/'+probe+'/registrations',token)).status===404,'UNKNOWN_ORDER_PRODUCTION_REGISTRATIONS_NOT_FOUND',report);
  ...
  report('PHASE_12_READ_ONLY_VERIFICATION: PASS');
}
async function main() {
  ...
  try {
    console.log('PRODUCTION_REGISTRATION_VERIFICATION_VERSION: 1');
