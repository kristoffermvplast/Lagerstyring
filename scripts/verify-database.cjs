...
const { clientTlsChecks } = require('./database-tls-check.cjs');
let stage = 'configuration';
async function verify() {
  if (!process.env.DATABASE_URL) throw new Error('DATABASE_URL_MISSING');
  const config = loadConfig();
  const tlsOnly = process.argv.includes('--tls-only');
  ...
  // Diagnostic mode reads the existing CA file without rewriting runtime files.
  const ssl = databaseTls(tlsOnly ? { ...config, DATABASE_CA_PEM: '' } : config);
  if (ssl.rejectUnauthorized !== true) throw new Error('TLS_REQUIRED');
  const pool = new Pool({ connectionString: config.DATABASE_URL, ssl, max: 1, connectionTimeoutMillis: 5000, query_timeout: 5000 });
  ...
  try {
    ...
    stage = 'client_tls';
    const checks = clientTlsChecks(client);
    for (const [name, passed] of Object.entries(checks)) {
      console.log(`${name}: ${passed ? 'PASS' : 'FAIL'}`);
    }
    if (Object.values(checks).some(value => value !== true)) throw new Error('TLS_CLIENT_CHECK_FAILED');
    // This measures the PostgreSQL-side hop, not the client-to-pooler socket.
    const observation = await client.query('select ssl from pg_stat_ssl where pid = pg_backend_pid()');
    const backendSsl = observation.rows[0]?.ssl;
    console.log(`postgres_backend_ssl: INFO (${backendSsl === true ? 'true' : backendSsl === false ? 'false' : 'unknown'}; PostgreSQL-side connection only)`);
    if (tlsOnly) return;
