...
async function verify(config, request = fetch, report = console.log) {
  ...
  try {
    ...
    requirePass(['inventory.read','inventory.reserve'].every(code => permissions.permissions?.some(p => p.code === code)), 'RESERVATION_PERMISSIONS', report);
    ...
    const prefix=`/companies/${membership.company_id}/reservations`,foreignPrefix=`/companies/${foreign}/reservations`;
    const response=await api(prefix+'?limit=1',token);requirePass(response.status===200,'RESERVATIONS_READ',report);
    const body=await response.json();requirePass(Array.isArray(body.items)&&body.items.every(r=>r.company_id===membership.company_id),'RESERVATIONS_SCOPE',report);
    const probe='00000000-0000-4000-8000-000000000000';
    for(const suffix of ['','/'+probe])requirePass((await api(foreignPrefix+suffix,token)).status===403,'RESERVATIONS_FOREIGN_DENIED',report);
    requirePass((await api(prefix+'/'+probe,token)).status===404,'UNKNOWN_RESERVATION_NOT_FOUND',report);
    if(body.items[0]){const response=await api(prefix+'/'+body.items[0].id,token);requirePass(response.status===200,'RESERVATION_DETAIL_READ',report);const detail=await response.json();requirePass(detail.company_id===membership.company_id&&detail.id===body.items[0].id&&Array.isArray(detail.events)&&detail.events.every(e=>e.company_id===membership.company_id&&e.reservation_id===detail.id),'RESERVATION_DETAIL_SCOPE',report);}
    else report('RESERVATION_EXISTING_RECORD: NOT_RUN (no existing reservation; no fixture created)');
  ...
  report('PHASE_17_READ_ONLY_VERIFICATION: PASS');
}
async function main() {
