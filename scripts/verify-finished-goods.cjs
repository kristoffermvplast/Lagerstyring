...
async function verify(config, request = fetch, report = console.log) {
  ...
  try {
    ...
    requirePass(['production.read','inventory.read','production.deliver','production.delivery.correct'].every(code => permissions.permissions?.some(p => p.code === code)), 'FINISHED_GOODS_PERMISSIONS', report);
    ...
    const wastePrefix=prefix+'/'+orderId+'/deliveries';
    const suffixes=['','/summary','/'+probe];
    for(const suffix of suffixes)requirePass((await api(foreignPrefix+'/'+orderId+'/deliveries'+suffix,token)).status===403,'FINISHED_GOODS_FOREIGN_DENIED_'+(suffix===''?'LIST':suffix==='/summary'?'ANALYSIS':'DETAIL'),report);
    const huRoot=`/companies/${membership.company_id}/handling-units`;
    const hr=await api(huRoot,token);requirePass(hr.status===200,'HANDLING_UNITS_READ',report);
    const units=await hr.json();requirePass(Array.isArray(units.items)&&units.items.every(x=>x.company_id===membership.company_id),'HANDLING_UNITS_SCOPE',report);
    for(const suffix of ['', '/'+probe, '/'+probe+'/moves'])requirePass((await api(`/companies/${foreign}/handling-units`+suffix,token)).status===403,'HANDLING_UNITS_FOREIGN_DENIED',report);
    if(units.items[0]){for(const suffix of ['','/moves'])requirePass((await api(huRoot+'/'+units.items[0].id+suffix,token)).status===200,'HANDLING_UNIT_DETAIL_READ',report);}
    else report('HANDLING_UNIT_EXISTING_RECORD: NOT_RUN (no existing handling unit; no fixture created)');
    if(body.items[0]) {
      const ar=await api(wastePrefix+'/summary',token);requirePass(ar.status===200,'FINISHED_GOODS_READ',report);
      const analysis=await ar.json();requirePass(analysis.company_id===membership.company_id&&analysis.order_id===orderId&&typeof analysis.remaining_quantity==='string','FINISHED_GOODS_SCOPE',report);
      const wr=await api(wastePrefix,token);requirePass(wr.status===200,'DELIVERY_HISTORY_READ',report);
      const history=await wr.json();requirePass(Array.isArray(history.items)&&history.items.every(x=>x.company_id===membership.company_id&&x.order_id===orderId),'DELIVERY_HISTORY_SCOPE',report);
      if(history.items[0]){const dr=await api(wastePrefix+'/'+history.items[0].id,token);requirePass(dr.status===200,'DELIVERY_DETAIL_READ',report);const d=await dr.json();requirePass(d.company_id===membership.company_id&&d.order_id===orderId,'DELIVERY_DETAIL_SCOPE',report);}
      else report('DELIVERY_EXISTING_RECORD: NOT_RUN (no existing delivery record; no fixture created)');
    } else report('FINISHED_GOODS_ORDER_FIXTURE: NOT_RUN (no existing production order; no fixture created)');
    for(const suffix of suffixes)requirePass((await api(prefix+'/'+probe+'/deliveries'+suffix,token)).status===404,'UNKNOWN_ORDER_DELIVERY_NOT_FOUND_'+(suffix===''?'LIST':suffix==='/summary'?'ANALYSIS':'DETAIL'),report);
  ...
  report('PHASE_15_READ_ONLY_VERIFICATION: PASS');
}
async function main() {
