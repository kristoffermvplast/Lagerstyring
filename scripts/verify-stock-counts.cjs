...
async function verify(config, request = fetch, report = console.log) {
  ...
  try {
    ...
    requirePass(['inventory.read','inventory.adjust','counts.read','counts.manage','counts.approve'].every(code => permissions.permissions?.some(p => p.code === code)), 'COUNT_PERMISSIONS', report);
    ...
    const prefix=`/companies/${membership.company_id}/stock-counts`,foreignPrefix=`/companies/${foreign}/stock-counts`;
    const probe='00000000-0000-4000-8000-000000000000';
    const response=await api(prefix+'?limit=1',token);
    requirePass(response.status===200,'COUNT_LIST_READ',report);
    const body=await response.json();
    requirePass(Array.isArray(body.items)&&body.items.every(c=>c.company_id===membership.company_id),'COUNT_RESPONSE_SCOPE',report);
    for(const suffix of ['', '/'+probe])requirePass((await api(foreignPrefix+suffix,token)).status===403,'COUNT_FOREIGN_DENIED',report);
    requirePass((await api(prefix+'/'+probe,token)).status===404,'UNKNOWN_COUNT_NOT_FOUND',report);
    const existing=body.items[0];
    if(existing){const r=await api(prefix+'/'+existing.id,token);requirePass(r.status===200,'COUNT_DETAIL_READ',report);const detail=await r.json();requirePass(detail.id===existing.id&&detail.company_id===membership.company_id,'COUNT_DETAIL_SCOPE',report);}
    else report('STOCK_COUNT_EXISTING_RECORD: NOT_RUN (no existing count; no fixture created)');
  ...
  report('PHASE_20_READ_ONLY_VERIFICATION: PASS');
}
async function main() {
  ...
  try {
    console.log('STOCK_COUNTS_VERIFICATION_VERSION: 1');
