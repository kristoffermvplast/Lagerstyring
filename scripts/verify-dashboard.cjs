...
async function verify(config, request = fetch, report = console.log) {
  ...
  try {
    ...
    requirePass(['dashboard.read','dashboard.acknowledge','production.read','inventory.read','masterdata.read','shipments.read','counts.read'].every(code => permissions.permissions?.some(p => p.code === code)), 'DASHBOARD_PERMISSIONS', report);
    ...
    const prefix=`/companies/${membership.company_id}/dashboard`;
    const response=await api(prefix,token);
    requirePass(response.status===200,'DASHBOARD_READ',report);
    const body=await response.json();
    requirePass(body.company_id===membership.company_id&&['alerts','orders','shipments','receipts','limited','permissions'].every(k=>Array.isArray(body[k])),'DASHBOARD_RESPONSE_SCOPE',report);
    const allowedPages=new Set(['Produktion','Forsendelser','Modtagelse','Lager','Optælling']);
    requirePass(body.alerts.every(a=>typeof a.key==='string'&&/^[0-9a-f]{64}$/.test(a.fingerprint)&&['open','acknowledged','resolved'].includes(a.state)&&allowedPages.has(a.target?.page)),'DASHBOARD_ALERT_SHAPE',report);
    requirePass((await api(`/companies/${foreign}/dashboard`,token)).status===403,'DASHBOARD_FOREIGN_DENIED',report);
    report(`DASHBOARD_COMPLETENESS: ${body.limited.length?'PARTIAL (bounded sources; inspect dashboard)':'PASS'}`);
    // No acknowledgement POST, business fixture, or source mutation is performed.
  ...
  report('PHASE_21_READ_ONLY_VERIFICATION: PASS');
}
async function main() {
  ...
  try {
    console.log('DASHBOARD_VERIFICATION_VERSION: 1');
