...
- Implement only the phase explicitly authorized by the user. Phase 8 (receiving through the existing inventory journal, receipt metadata, quantity differences and access boundaries) is authorized. Do not implement Phase 9 transfer workflows or later features.
...
- Economic boundary: At most 1 DKK expected new/increased external charge per concrete action without separate approval. If the cost exceeds 1 DKK or cannot reasonably be bounded at 1 DKK, ask before acting. No plan/resource/limit changes, new paid services or credits. Before push/hosted migration/deploy, establish the action stays within that boundary or ask approval. Use local test fixtures; never create hosted test tenants without approval.
