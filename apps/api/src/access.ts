...
const roleBody = z.object({ name: z.string().trim().min(1).max(80), permissions: z.array(z.enum(['access.read','access.manage'])).max(2) }).strict();
...
export class AccessController {
  ...
  async createRole(@Req() req: { actor: Actor }, @Param('companyId') id: string, @Body() body: unknown) {
    ...
    if (input.permissions.includes('access.manage') && !input.permissions.includes('access.read')) throw new BadRequestException();
    return this.database.asActor(req.actor,parse(uuid,id),async client => {
      await authorize(client,'access.manage');
