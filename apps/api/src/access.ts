...
const roleBody = z.object({ name: z.string().trim().min(1).max(80), permissions: z.array(z.enum(['access.read','access.manage','masterdata.read','masterdata.manage','inventory.read','inventory.adjust','inventory.receive'])).max(7) }).strict();
