...
const roleBody = z.object({ name: z.string().trim().min(1).max(80), permissions: z.array(z.enum(['access.read','access.manage','masterdata.read','masterdata.manage','inventory.read','inventory.adjust','inventory.receive','inventory.transfer','production.read','production.manage','production.issue','production.record','production.correct','production.return','production.close','production.waste','production.waste.correct','production.deliver','production.delivery.correct'])).max(17) }).strict();
