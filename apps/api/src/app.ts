...
const FOUNDATION_PUBLIC = 'foundation-public-health';

@Injectable()
class PhaseOneGuard implements CanActivate {
  constructor(@Inject(Reflector) private readonly reflector: Reflector) {}
  canActivate(context: ExecutionContext) {
    if (this.reflector.get<boolean>(FOUNDATION_PUBLIC, context.getHandler())) return true;
    // Authentication and permissions are explicitly deferred to Phase 2.
    // Future controllers are closed by default, even if a caller sends a token.
...
class HealthController {
  ...
  @Get('live')
  @SetMetadata(FOUNDATION_PUBLIC, true)
...
export async function createApp(config: AppConfig, database = new DatabaseService(config)) {
  const app = await NestFactory.create<NestExpressApplication>({
    ...
    providers: [{ provide: DatabaseService, useValue: database }, { provide: APP_GUARD, useClass: PhaseOneGuard }],
  ...
  app.enableCors({ origin: config.origins, credentials: false, methods: ['GET', 'HEAD', 'OPTIONS'], allowedHeaders: ['Content-Type', 'Authorization'] });
  ...
  if (config.NODE_ENV !== 'production') {
    const doc = SwaggerModule.createDocument(app, new DocumentBuilder().setTitle('Lagerstyring · Foundation API').setVersion('0.1.0').build());
