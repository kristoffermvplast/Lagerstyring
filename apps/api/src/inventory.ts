...
export class InventoryController {
 ...
 @Get('balances')
 balances(@Req()r:{actor:Actor},@Param('companyId')company:string,@Query()raw:unknown){const q=parse(filters,raw);return this.run(r.actor,company,false,async db=>{const p=[company,q.item_id??null,q.owner_id??null,q.location_id??null,'%'+q.q.replace(/[\\%_]/g,'\\$&')+'%'];const where="company_id=$1 and ($2::uuid is null or item_id=$2) and ($3::uuid is null or owner_id=$3) and ($4::uuid is null or location_id=$4) and (snapshot->'item'->>'code' ilike $5 or snapshot->'item'->>'name' ilike $5)";const total=(await db.query('select count(*)::int total from app.stock_balances where '+where,p)).rows[0].total;return{items:(await db.query('select *, (quantity-reserved_quantity)::text as available_quantity from app.stock_balances where '+where+' order by item_id,owner_id,location_id limit $6 offset $7',[...p,q.limit,(q.page-1)*q.limit])).rows,total,page:q.page,limit:q.limit};});}
 @Get('entries')
