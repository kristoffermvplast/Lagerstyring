import {QuickActions} from './VisualPrimitives';
import {WorkspaceNavigation,focusMenu} from './WorkspaceNavigation';
...
function Workspace({localLogout}:{localLogout:()=>void}) {
  ...
  const canProductionRead=access.data?.permissions.some(p=>p.code==='production.read')??false;
  const canProductionManage=access.data?.permissions.some(p=>p.code==='production.manage')??false;
  const canInventoryRead=access.data?.permissions.some(p=>p.code==='inventory.read')??false;
  const canForecast=canMasterRead&&canInventoryRead&&canProductionRead&&(access.data?.permissions.some(p=>p.code==='shipments.read')??false);
  const canTransfer=access.data?.permissions.some(p=>p.code==='inventory.transfer')??false;
  const canReceive=access.data?.permissions.some(p=>p.code==='inventory.receive')??false;
  ...
  const pages=['Overblik',...(canForecast?['Prognose']:[]),...(canMasterRead&&(canMasterManage||(canInventoryRead&&canInventoryAdjust))?['Import']:[]),...(access.data?.permissions.some(p=>p.code==='reports.read')?['Rapporter']:[]),...(canInventoryRead&&access.data?.permissions.some(p=>p.code==='counts.read')?['Optælling']:[]),...(access.data?.permissions.some(p=>p.code==='pallets.read')?['Pallemellemværender']:[]),...(access.data?.permissions.some(p=>p.code==='shipments.read')?['Forsendelser']:[]),...(id?['Scan QR']:[]),...(canProductionRead?['Produktion']:[]),...(canInventoryRead?['Lager','Modtagelse','Lagerflytning','Reservationer']:[]),...(canInventoryRead&&canProductionRead?['Pallestyring']:[]),...(canMasterRead?[...Object.keys(itemPages),'Styklister og pakning','Lagerplaceringer',...Object.keys(masterdataPages)]:[]),'Min profil',...(canRead?['Adgang']:[])];
  return <div className="access-shell"><a className="skip-link" href="#content">Gå til indhold</a><header className="access-header"><div className="auth-brand"><Boxes />Lagerstyring</div><span className="header-context">{selected?.name ?? 'Arbejdsrum'}<span aria-hidden="true"> / </span><strong>{page}</strong></span><div className="header-actions"><span className="user-avatar" aria-label={me.data?.user.display_name || 'Din profil'}>{(me.data?.user.display_name || 'U').slice(0,1).toUpperCase()}</span></div></header>
    <ConnectionStatus/><div className="access-body"><WorkspaceNavigation key={id} pages={pages} current={page} choose={name=>{setScan(null);setTarget(null);setPage(name);document.getElementById("content")?.focus();}}><label>Virksomhed<select aria-label="Virksomhed" value={id ?? ''} onChange={e=>{setCompany(e.target.value);setScan(null);setTarget(null);setPage('Overblik');cache.removeQueries({queryKey:['members']});cache.removeQueries({queryKey:['roles']});}}>
      {!memberships.length && <option value="">Ingen virksomhed</option>}{memberships.map(m=><option key={m.company_id} value={m.company_id}>{m.name}</option>)}</select></label><span className="nav-user">{me.data?.user.display_name}</span><button onClick={logout} disabled={busy}>Log ud</button>
</WorkspaceNavigation>
    <main id="content" tabIndex={-1} className="access-content" onInvalidCapture={e=>{let parent=(e.target as HTMLElement).parentElement;while(parent){if(parent instanceof HTMLDetailsElement)parent.open=true;parent=parent.parentElement;}}}><button className="workspace-menu-return" onClick={focusMenu}>Gå til menu</button><h1 aria-label={page}>{page==='Overblik'&&me.data ? `Hej${me.data.user.display_name ? ', '+me.data.user.display_name : ''}` : page}</h1>{error && <div role="alert"><p>{error}</p><button onClick={localLogout}>Luk kun sessionen på denne enhed</button><p>Serverens logout er ikke bekræftet, hvis forbindelsen fejlede.</p></div>}{me.isPending && <p role="status">Henter dit arbejdsrum…</p>}{me.error && <p role="alert">{message(me.error)}</p>}{access.error && <p role="alert">{message(access.error)}</p>}
      {id&&page==='Prognose'&&canForecast&&<Forecast key={id} company={id}/>}
      ...
      {me.data && page==='Overblik' && <section className="dashboard-welcome">{!id&&<p>Du har endnu ingen aktiv virksomhedsadgang. Kontakt en administrator og oplys dit bruger-ID fra Min profil.</p>}<p>Lager, produktion og forsendelser — samlet ét sted.</p></section>}
      {id && page==='Overblik' && <QuickActions pages={pages} choose={name=>{setScan(null);setTarget(null);setPage(name);}}/>}
      {id && page==='Import' && <Imports key={id} company={id} permissions={access.data?.permissions.map(p=>p.code)??[]}/>}
      {id && page==='Rapporter' && access.data?.permissions.some(p=>p.code==='reports.read') && <Reports key={id} company={id} permissions={access.data.permissions.map(p=>p.code)} open={t=>{setScan(null);setTarget(t);setPage(t.page);}}/>}
      {id && page==='Overblik' && access.data?.permissions.some(p=>p.code==='dashboard.read') && <Dashboard key={id} company={id} open={t=>{setScan(null);setTarget(t);setPage(t.page);}}/>}
      ...
      {id && canInventoryRead && page==='Modtagelse' && <Receiving initialId={target?.id} key={id+':'+(target?.id??target?.q??'')} company={id} canReceive={canReceive} canAdjust={canInventoryAdjust} canMasterRead={canMasterRead}/>}
      {id && canInventoryRead && page==='Optælling' && access.data?.permissions.some(p=>p.code==='counts.read') && <StockCounts initialId={target?.id} key={id+':'+(target?.id??target?.q??'')} company={id} permissions={access.data.permissions.map(p=>p.code)}/>}
      {id && page==='Pallemellemværender' && access.data?.permissions.some(p=>p.code==='pallets.read') && <PalletAccounts key={id} company={id} permissions={access.data.permissions.map(p=>p.code)}/>}
      {id && page==='Forsendelser' && access.data?.permissions.some(p=>p.code==='shipments.read') && <Shipments initialId={target?.id} key={id+':'+(target?.id??target?.q??'')} company={id} permissions={access.data.permissions.map(p=>p.code)}/>}
      {id && canInventoryRead && page==='Reservationer' && <Reservations key={id} company={id} canReserve={access.data?.permissions.some(p=>p.code==='inventory.reserve')??false} canMasterRead={canMasterRead} canProductionRead={canProductionRead}/>}
      {id && canInventoryRead && page==='Lager' && <Inventory initialEntryId={target?.id} initialQuery={target?.q} key={id+':'+(target?.id??target?.q??'')} company={id} canAdjust={canInventoryAdjust} canMasterRead={canMasterRead}/>}
...
function Administration({company,canManage}:{company:string;canManage:boolean}) {
  ...
  const [editing,setEditing]=useState(''); const [roleName,setRoleName]=useState(''); const [read,setRead]=useState(false); const [manage,setManage]=useState(false); const [masterRead,setMasterRead]=useState(false); const [masterManage,setMasterManage]=useState(false); const [inventoryRead,setInventoryRead]=useState(false); const [inventoryAdjust,setInventoryAdjust]=useState(false); const [receive,setReceive]=useState(false); const [transfer,setTransfer]=useState(false); const [productionRead,setProductionRead]=useState(false); const [productionManage,setProductionManage]=useState(false); const [productionActions,setProductionActions]=useState<string[]>([]);
  const actionLabels:Record<string,string>={'reports.read':'Se rapporter','reports.export':'Eksportere rapporter','dashboard.read':'Se dashboard','dashboard.acknowledge':'Kvittere egne advarsler','counts.read':'Se optællinger','counts.manage':'Registrere optællinger','counts.approve':'Godkende optællinger','pallets.read':'Se pallemellemværender','pallets.manage':'Registrere pallebevægelser','pallets.adjust':'Korrigere pallemellemværender','shipments.read':'Se forsendelser','shipments.manage':'Planlægge forsendelser','shipments.dispatch':'Afsende forsendelser','production.deliver':'Aflevere færdigvarer','production.delivery.correct':'Modpostere færdigvareaflevering','inventory.reserve':'Reservere og frigive lager','production.issue':'Sende materiale til produktion','production.record':'Registrere gode emner','production.correct':'Modregistrere gode emner','production.return':'Returnere materiale','production.close':'Afstemme og afslutte produktion','production.waste':'Registrere målt fysisk spild','production.waste.correct':'Modregistrere fysisk spild'};
  ...
  <section className="access-card"><h2>Roller og rettigheder</h2>{roles.data?.map(r=><div className="role-row" key={r.id}><strong>{r.name}</strong><span>{r.is_admin?'Alle virksomhedens rettigheder':r.permissions.join(', ')||'Ingen administrationsrettigheder'}</span>{canManage&&!r.is_admin&&<button onClick={()=>{setEditing(r.id);setRoleName(r.name);setProductionActions(r.permissions.filter(p=>p in actionLabels));setRead(r.permissions.includes('access.read'));setManage(r.permissions.includes('access.manage'));setMasterRead(r.permissions.includes('masterdata.read'));setMasterManage(r.permissions.includes('masterdata.manage'));setInventoryRead(r.permissions.includes('inventory.read'));setInventoryAdjust(r.permissions.includes('inventory.adjust'));setReceive(r.permissions.includes('inventory.receive'));setTransfer(r.permissions.includes('inventory.transfer'));setProductionRead(r.permissions.includes('production.read'));setProductionManage(r.permissions.includes('production.manage'));}}>Redigér</button>}</div>)}
  {canManage&&<form onSubmit={e=>{e.preventDefault();void mutate(editing?`/roles/${editing}`:'/roles',editing?'PATCH':'POST',{name:roleName,permissions:[...(read?['access.read']:[]),...(manage?['access.manage']:[]),...(masterRead?['masterdata.read']:[]),...(masterManage?['masterdata.manage']:[]),...(inventoryRead?['inventory.read']:[]),...(inventoryAdjust?['inventory.adjust']:[]),...(receive?['inventory.receive']:[]),...(transfer?['inventory.transfer']:[]),...(productionRead?['production.read']:[]),...(productionManage?['production.manage']:[]),...productionActions]});}}><h3>{editing?'Redigér rolle':'Ny rolle'}</h3><label>Rollenavn<input required maxLength={80} value={roleName} onChange={e=>setRoleName(e.target.value)}/></label><label className="check"><input type="checkbox" checked={read} onChange={e=>{setRead(e.target.checked);if(!e.target.checked)setManage(false);}}/>Se brugere og roller</label><label className="check"><input type="checkbox" checked={manage} onChange={e=>{setManage(e.target.checked);if(e.target.checked)setRead(true);}}/>Administrér adgang, inklusive administratorroller</label><label className="check"><input type="checkbox" checked={masterRead} onChange={e=>{setMasterRead(e.target.checked);if(!e.target.checked){setMasterManage(false);setProductionManage(false);}}}/>Se stamdata og historik</label><label className="check"><input type="checkbox" checked={masterManage} onChange={e=>{setMasterManage(e.target.checked);if(e.target.checked)setMasterRead(true);}}/>Oprette og ændre stamdata</label><label className="check"><input type="checkbox" checked={inventoryRead} onChange={e=>{setInventoryRead(e.target.checked);if(!e.target.checked){setInventoryAdjust(false);setReceive(false);setTransfer(false);}}}/>Se lager og posteringer</label><label className="check"><input type="checkbox" checked={inventoryAdjust} onChange={e=>{setInventoryAdjust(e.target.checked);if(e.target.checked)setInventoryRead(true);}}/>Lagerkorrektioner og ejere</label><label className="check"><input type="checkbox" checked={receive} onChange={e=>{setReceive(e.target.checked);if(e.target.checked)setInventoryRead(true);}}/>Registrere modtagelser</label><label className="check"><input type="checkbox" checked={transfer} onChange={e=>{setTransfer(e.target.checked);if(e.target.checked)setInventoryRead(true);}}/>Flytte lager</label><label className="check"><input type="checkbox" checked={productionRead} onChange={e=>{setProductionRead(e.target.checked);if(!e.target.checked)setProductionManage(false);}}/>Se produktionsordrer og behov</label><label className="check"><input type="checkbox" checked={productionManage} onChange={e=>{setProductionManage(e.target.checked);if(e.target.checked){setProductionRead(true);setMasterRead(true);}}}/>Planlægge produktion</label>{Object.entries(actionLabels).map(([code,label])=><label className="check" key={code}><input type="checkbox" checked={productionActions.includes(code)} onChange={e=>{setProductionActions(old=>e.target.checked?[...old,code]:old.filter(x=>x!==code));if(e.target.checked){if(code.startsWith('reports.')){if(code==='reports.export')setProductionActions(old=>[...new Set([...old,'reports.read'])]);}else if(code.startsWith('dashboard.')){if(code==='dashboard.acknowledge')setProductionActions(old=>[...new Set([...old,'dashboard.read'])]);}else if(code.startsWith('pallets.')){if(code!=='pallets.read'){setProductionActions(old=>Array.from(new Set([...old,'pallets.read'])));setMasterRead(true);}}else if(code.startsWith('shipments.')){if(code!=='shipments.read'){setProductionActions(old=>Array.from(new Set([...old,'shipments.read'])));setInventoryRead(true);setMasterRead(true);}}else if(code!=='inventory.reserve')setProductionRead(true);else setInventoryRead(true);if(['production.issue','production.return','production.close'].includes(code)){setInventoryRead(true);setMasterRead(true);}if(['production.issue','production.return'].includes(code))setTransfer(true);if(code==='production.close')setProductionManage(true);if(code.startsWith('production.waste')||code.startsWith('production.deliver'))setInventoryRead(true);if(code==='production.deliver')setMasterRead(true);}}}/>{label}</label>)}<button disabled={busy}>{editing?'Gem rolle':'Opret rolle'}</button>{editing&&<button type="button" onClick={()=>{setEditing('');setRoleName('');setProductionActions([]);setRead(false);setManage(false);setMasterRead(false);setMasterManage(false);setInventoryRead(false);setInventoryAdjust(false);setReceive(false);setTransfer(false);setProductionRead(false);setProductionManage(false);}}>Annullér redigering</button>}</form>}</section>
