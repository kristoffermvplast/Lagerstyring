...
function Workspace({localLogout}:{localLogout:()=>void}) {
  ...
  return <div className="access-shell"><a className="skip-link" href="#content">Gå til indhold</a><header className="access-header"><div className="auth-brand"><Boxes />Lagerstyring</div><button onClick={logout} disabled={busy}>Log ud</button></header>
    ...
    <main id="content" tabIndex={-1} className="access-content" onInvalidCapture={e=>{let parent=(e.target as HTMLElement).parentElement;while(parent){if(parent instanceof HTMLDetailsElement)parent.open=true;parent=parent.parentElement;}}}><button className="workspace-menu-return" onClick={focusMenu}>Gå til menu</button><h1>{page}</h1><p className="form-guidance">Felter med mørk venstrekant skal udfyldes.</p>{error && <div role="alert"><p>{error}</p><button onClick={localLogout}>Luk kun sessionen på denne enhed</button><p>Serverens logout er ikke bekræftet, hvis forbindelsen fejlede.</p></div>}{me.isPending && <p role="status">Henter dit arbejdsrum…</p>}{me.error && <p role="alert">{message(me.error)}</p>}{access.error && <p role="alert">{message(access.error)}</p>}
