...
function Workspace({localLogout}:{localLogout:()=>void}) {
  ...
  return <div className="access-shell"><a className="skip-link" href="#content">Gå til indhold</a><header className="access-header"><div className="auth-brand"><Boxes />Lagerstyring</div><button onClick={logout} disabled={busy}>Log ud</button></header>
    <div className="access-body"><aside className="access-nav"><label>Virksomhed<select aria-label="Virksomhed" value={id ?? ''} onChange={e=>{setCompany(e.target.value);setPage('Overblik');cache.removeQueries({queryKey:['members']});cache.removeQueries({queryKey:['roles']});}}>
      ...
      {['Overblik',...(canInventoryRead?['Lager','Modtagelse']:[]),...(canMasterRead?[...Object.keys(itemPages),'Styklister og pakning','Lagerplaceringer',...Object.keys(masterdataPages)]:[]),'Min profil',...(canRead?['Adgang']:[])].map(name=><button key={name} aria-current={page===name?'page':undefined} onClick={()=>setPage(name)}>{name}</button>)}</aside>
...
function Administration({company,canManage}:{company:string;canManage:boolean}) {
  ...
  <section className="access-card"><h2>Roller og rettigheder</h2>{roles.data?.map(r=><div className="role-row" key={r.id}><strong>{r.name}</strong><span>{r.is_admin?'Alle virksomhedens rettigheder':r.permissions.join(', ')||'Ingen administrationsrettigheder'}</span>{canManage&&!r.is_admin&&<button onClick={()=>{setEditing(r.id);setRoleName(r.name);setRead(r.permissions.includes('access.read'));setManage(r.permissions.includes('access.manage'));setMasterRead(r.permissions.includes('masterdata.read'));setMasterManage(r.permissions.includes('masterdata.manage'));setInventoryRead(r.permissions.includes('inventory.read'));setInventoryAdjust(r.permissions.includes('inventory.adjust'));setReceive(r.permissions.includes('inventory.receive'));}}>Redigér</button>}</div>)}
  {canManage&&<form onSubmit={e=>{e.preventDefault();void mutate(editing?`/roles/${editing}`:'/roles',editing?'PATCH':'POST',{name:roleName,permissions:[...(read?['access.read']:[]),...(manage?['access.manage']:[]),...(masterRead?['masterdata.read']:[]),...(masterManage?['masterdata.manage']:[]),...(inventoryRead?['inventory.read']:[]),...(inventoryAdjust?['inventory.adjust']:[]),...(receive?['inventory.receive']:[])]});}}><h3>{editing?'Redigér rolle':'Ny rolle'}</h3><label>Rollenavn<input required maxLength={80} value={roleName} onChange={e=>setRoleName(e.target.value)}/></label><label className="check"><input type="checkbox" checked={read} onChange={e=>{setRead(e.target.checked);if(!e.target.checked)setManage(false);}}/>Se brugere og roller</label><label className="check"><input type="checkbox" checked={manage} onChange={e=>{setManage(e.target.checked);if(e.target.checked)setRead(true);}}/>Administrér adgang, inklusive administratorroller</label><label className="check"><input type="checkbox" checked={masterRead} onChange={e=>{setMasterRead(e.target.checked);if(!e.target.checked)setMasterManage(false);}}/>Se stamdata og historik</label><label className="check"><input type="checkbox" checked={masterManage} onChange={e=>{setMasterManage(e.target.checked);if(e.target.checked)setMasterRead(true);}}/>Oprette og ændre stamdata</label><label className="check"><input type="checkbox" checked={inventoryRead} onChange={e=>{setInventoryRead(e.target.checked);if(!e.target.checked){setInventoryAdjust(false);setReceive(false);}}}/>Se lager og posteringer</label><label className="check"><input type="checkbox" checked={inventoryAdjust} onChange={e=>{setInventoryAdjust(e.target.checked);if(e.target.checked)setInventoryRead(true);}}/>Lagerkorrektioner og ejere</label><label className="check"><input type="checkbox" checked={receive} onChange={e=>{setReceive(e.target.checked);if(e.target.checked)setInventoryRead(true);}}/>Registrere modtagelser</label><button disabled={busy}>{editing?'Gem rolle':'Opret rolle'}</button>{editing&&<button type="button" onClick={()=>{setEditing('');setRoleName('');setRead(false);setManage(false);setMasterRead(false);setMasterManage(false);setInventoryRead(false);setInventoryAdjust(false);setReceive(false);}}>Annullér redigering</button>}</form>}</section>
