...
function Workspace({localLogout}:{localLogout:()=>void}) {
  ...
  return <div className="access-shell"><a className="skip-link" href="#content">Gå til indhold</a><header className="access-header"><div className="auth-brand"><Boxes />Lagerstyring</div><button onClick={logout} disabled={busy}>Log ud</button></header>
    <div className="access-body"><aside className="access-nav"><label>Virksomhed<select aria-label="Virksomhed" value={id ?? ''} onChange={e=>{setCompany(e.target.value);setPage('Overblik');cache.removeQueries({queryKey:['members']});cache.removeQueries({queryKey:['roles']});}}>
      ...
      {['Overblik','Min profil',...(canRead?['Adgang']:[])].map(name=><button key={name} aria-current={page===name?'page':undefined} onClick={()=>setPage(name)}>{name}</button>)}</aside>
...
function Administration({company,canManage}:{company:string;canManage:boolean}) {
  ...
  const [editing,setEditing]=useState(''); const [roleName,setRoleName]=useState(''); const [read,setRead]=useState(false); const [manage,setManage]=useState(false);
  ...
  <section className="access-card"><h2>Roller og rettigheder</h2>{roles.data?.map(r=><div className="role-row" key={r.id}><strong>{r.name}</strong><span>{r.is_admin?'Alle virksomhedens rettigheder':r.permissions.join(', ')||'Ingen administrationsrettigheder'}</span>{canManage&&!r.is_admin&&<button onClick={()=>{setEditing(r.id);setRoleName(r.name);setRead(r.permissions.includes('access.read'));setManage(r.permissions.includes('access.manage'));}}>Redigér</button>}</div>)}
  {canManage&&<form onSubmit={e=>{e.preventDefault();void mutate(editing?`/roles/${editing}`:'/roles',editing?'PATCH':'POST',{name:roleName,permissions:[...(read?['access.read']:[]),...(manage?['access.manage']:[])]});}}><h3>{editing?'Redigér rolle':'Ny rolle'}</h3><label>Rollenavn<input required maxLength={80} value={roleName} onChange={e=>setRoleName(e.target.value)}/></label><label className="check"><input type="checkbox" checked={read} onChange={e=>{setRead(e.target.checked);if(!e.target.checked)setManage(false);}}/>Se brugere og roller</label><label className="check"><input type="checkbox" checked={manage} onChange={e=>{setManage(e.target.checked);if(e.target.checked)setRead(true);}}/>Administrér adgang, inklusive administratorroller</label><button disabled={busy}>{editing?'Gem rolle':'Opret rolle'}</button>{editing&&<button type="button" onClick={()=>{setEditing('');setRoleName('');setRead(false);setManage(false);}}>Annullér redigering</button>}</form>}</section>
